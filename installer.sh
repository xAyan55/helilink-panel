...
ni_header() {
    printf "\n"
    printf " _   _ _____ _     ___ _     ___ _   _ _  __\n"
    printf "| | | | ____| |   |_ _| |   |_ _| \\ | | |/ /\n"
    printf "| |_| |  _| | |    | || |    | ||  \\| | ' / \n"
    printf "|  _  | |___| |___ | || |___ | || |\\  | . \\ \n"
    printf "|_| |_|_____|_____|___|_____|___|_| \\_|_|\\_\\\\\n"
    printf "\n"
    printf "  ${BOLD}HeliLink Installer${RESET} ${C_GRAY}v${VERSION}${RESET}  ${C_GRAY}%s${RESET}\n\n" "$(date '+%Y-%m-%d %H:%M:%S')"
}
...
tui_progress_draw() {
    ...
    move_to $(( box_r + 1 )) $(( box_c + 3 ))
    printf "${DIM}HeliLink v${VERSION}${RESET}"
    tui_hline $(( box_r + 2 )) "$box_c" "$box_w"
...
run_interactive() {
    ...
    tui_cleanup
    printf "\n  HeliLink Installer v${VERSION} — done\n\n"
}
...
touch "$LOG" || true
log "=== HeliLink Installer v${VERSION} started (pid $$) ==="
