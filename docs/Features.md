# HeliLink Panel - Feature Guide

This is a reference for everything in the HeliLink panel. It covers the admin side, the user side, the addon system, and the API.
...
Nodes are the machines that run game servers. Each node runs the HeliLink daemon, which the panel communicates with.
